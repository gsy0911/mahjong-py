...
class Ju:
    ...
    def get_wang_pai_and_fixed_bi_pai(
        ...
        if dice_sum in [5, 9]:
            fixed_bi_pai.extend(bi_pai_dong[dice_sum:])
            ...
            fixed_bi_pai.extend(bi_pai_dong)
            fixed_bi_pai.extend(bi_pai_bei)
            fixed_bi_pai.extend(bi_pai_xi)
            fixed_bi_pai.extend(bi_pai_nan[:dice_sum])
    ...
    def get_initial_pei_pai_and_rest_pai(
        ...
        Returns: dong, nan, xi, bei, rest_fixed_bi_pai
        """
        ...
        for i in range(8 * 3 + 3):
            if i in [0, 1, 8, 9, 16, 17]:
                pei_pai_dong.extend(fixed_bi_pai[i].get_pai())
                ...
                pei_pai_dong.append(fixed_bi_pai[i].upper_pai)
        ...
        return pei_pai_dong, pei_pai_nan, pei_pai_xi, pei_pai_bei, rest_fixed_bi_pai
